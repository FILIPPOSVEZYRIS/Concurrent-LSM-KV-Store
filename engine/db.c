...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    //pthread_mutex_init(&self->my_lock,NULL); //mutex initialization
   
    
    pthread_mutex_init(&self->rw_lock, NULL); //mutex rw initialization
    pthread_cond_init(&self->readers_cv, NULL); //initialization
    pthread_cond_init(&self->writers_cv, NULL); //initialization
    self->active_readers = 0; //set counters to 0
...
void db_close(DB *self)
{
    ...
    //pthread_mutex_destroy(&self->my_lock); //mutex destroy
    pthread_mutex_destroy(&self->rw_lock);
    pthread_cond_destroy(&self->readers_cv);
    pthread_cond_destroy(&self->writers_cv);
...
{
    //pthread_mutex_lock(&self->my_lock); //mutex lock
    
    pthread_mutex_lock(&self->rw_lock); //mutex lock
    
    self->waiting_writers++; //a writer is added to the waiting list
    
    //we wait while there is some other active reader or an active writer
    while (self->active_readers > 0 || self->active_writers > 0) {
        pthread_cond_wait(&self->writers_cv, &self->rw_lock);
    }
    
    self->waiting_writers--; //a writer is no longer in the waiting list
    self->active_writers++;  //a writer starts writing
    
    pthread_mutex_unlock(&self->rw_lock); //mutex unlock
    ...
    //pthread_mutex_unlock(&self->my_lock); //mutex unlock
    
    
    
    pthread_mutex_lock(&self->rw_lock); //mutex lock
    
    self->active_writers--; // writing ended
    
    //other writers have priority
    if (self->waiting_writers > 0) {
        pthread_cond_signal(&self->writers_cv); // we wake up a writer if he is waiting
    } else {
        pthread_cond_broadcast(&self->readers_cv); //we wake up all the readers if there are no writers waiting
    }
...
int db_get(DB* self, Variant* key, Variant* value)
{
    ...
    //we wait while someone writes or is waiting to write
    while (self->active_writers > 0 || self->waiting_writers > 0) {
        pthread_cond_wait(&self->readers_cv, &self->rw_lock);
    }
    self->active_readers++; //a reader enters
    
    pthread_mutex_unlock(&self->rw_lock); //unlock for other readers to enter
    ...
    self->active_readers--; //a reader has finished reading
    
    //if i was the last reader I wake up a writer that(if) is waiting
    if (self->active_readers == 0 && self->waiting_writers > 0) {
        pthread_cond_signal(&self->writers_cv);
    }
